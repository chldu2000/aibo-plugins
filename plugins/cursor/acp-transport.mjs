...
export class AcpTransport {
  constructor({ command = 'agent', args = ['acp'], cwd, spawnProcess = spawn, requestTimeoutMs = 14_000 } = {}) {
  ...
  start() {
    ...
    child.once('exit', (code, signal) => {
      ...
      this.#finish(new Error(`Cursor ACP exited (${signal ?? code ?? 'unknown'})${detail ? `: ${detail}` : ''}`));
    });
    return this;
  }
  ...
  #finish(error) {
    ...
    for (const handler of this.notificationHandlers) handler({ jsonrpc: '2.0', method: 'transport/closed', params: { message: error.message } });
  }
